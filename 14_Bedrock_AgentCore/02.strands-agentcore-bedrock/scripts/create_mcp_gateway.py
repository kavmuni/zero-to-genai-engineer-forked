...
2) Export for deploy (see this lab's README Demo 3+):
     export GATEWAY_URL="{gateway.get("gatewayUrl")}"
     export GATEWAY_TOKEN="$(.venv/bin/python scripts/get_gateway_token.py)"

3) Create Identity provider in Console (us-east-1) named gateway-cognito-m2m
   using cognito.token_endpoint / client_id / client_secret / scope from {out.name}.

4) Deploy your gateway/identity agent with YOUR MEMORY_ID from .env — never a
   shared classroom Memory id. Follow the lab README step by step.
================================================================================
"""
    )
