...
Reads ./gateway-credentials.json by default (created by scripts/create_mcp_gateway.py
in THIS lab folder). Falls back to CLI flags / env vars for an existing Cognito pool.
...
def load_bundle(path: Path) -> dict:
    if not path.is_file():
        raise FileNotFoundError(
            f"Missing {path}\n"
            "Create a gateway first (this lab folder):\n"
